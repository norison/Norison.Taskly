...
public class DeleteTodoCommandHandler(IRepository<Todo> todosRepository, IDateNowService dateNowService)
    : ICommandHandler<DeleteTodoCommand, TodoDto>
{
    public async Task<TodoDto> Handle(DeleteTodoCommand request, CancellationToken cancellationToken)
    {
        ...
        todo.Delete(dateNowService.Now);
